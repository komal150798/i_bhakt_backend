...
/**
 * SME Astrology Rulebook schema.
 * Step 20 Data Model sections 65-73, Step 10 section 19.
 *
 * Creates the externally managed knowledge layer required by Step 20
 * section 122 and Step 10 section 47: astrology rules live in versioned,
 * uploadable data - never in source code or prompts.
 *
 * Idempotent throughout, so it is safe against a database where TypeORM
 * synchronize already created the tables.
 */
export class CreateZunoRulebook1757700000000 implements MigrationInterface {
  name = 'CreateZunoRulebook1757700000000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`CREATE EXTENSION IF NOT EXISTS "pgcrypto"`);

    // ---------------------------------------------------------------
    // Rulebook versions (Step 20 section 65)
    // ---------------------------------------------------------------
    ...
    `);
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_zuno_rulebook_versions_version" ON "zuno_rulebook_versions" ("version")`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "idx_zuno_rulebook_versions_status" ON "zuno_rulebook_versions" ("status")`,
    );
    // Step 10 section 27 / Step 20 section 69: exactly one active production
    // rulebook. A partial unique index makes that a database guarantee rather
    // than something the service layer must never get wrong - two concurrent
    ...
    `);
    // Detects a duplicate upload of an identical file (Step 10 section 10).
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_zuno_rulebook_file_hash" ON "zuno_rulebook_versions" ("source_file_hash")`,
    );

    // ---------------------------------------------------------------
    // Rules (Step 20 section 67)
    // ---------------------------------------------------------------
    await queryRunner.query(`
    ...
    `);
    // Step 10 section 13: a rule ID is permanent and unique within its version.
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_zuno_rules_version_key" ON "zuno_rulebook_rules" ("rulebook_version_id", "external_rule_key")`,
    );
    // Step 20 section 87: the runtime lookup path is (version, domain, status).
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "idx_zuno_rules_domain" ON "zuno_rulebook_rules" ("rulebook_version_id", "domain", "status")`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "idx_zuno_rules_theme" ON "zuno_rulebook_rules" ("rulebook_version_id", "theme")`,
    );
    // Step 20 section 88: the hot path only ever reads approved rules.
    await queryRunner.query(`
    ...
    // ---------------------------------------------------------------
    // Interpretations, timing, remedies, domain config, conflicts, golden cases
    // ---------------------------------------------------------------
    await queryRunner.query(`
    ...
    `);
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_zuno_interpretations_key" ON "zuno_rulebook_interpretations" ("rulebook_version_id", "external_interpretation_key")`,
    );
    ...
    `);
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_zuno_timing_key" ON "zuno_rulebook_timing_rules" ("rulebook_version_id", "external_timing_key")`,
    );
    ...
    `);
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_zuno_remedies_key" ON "zuno_rulebook_remedies" ("rulebook_version_id", "external_remedy_key")`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "idx_zuno_remedies_type" ON "zuno_rulebook_remedies" ("rulebook_version_id", "remedy_type")`,
    );
    ...
    `);
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_zuno_domain_config" ON "zuno_rulebook_domain_configs" ("rulebook_version_id", "domain")`,
    );
    ...
    `);
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_zuno_conflict_key" ON "zuno_rulebook_conflict_rules" ("rulebook_version_id", "external_conflict_key")`,
    );
    ...
    `);
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_zuno_golden_key" ON "zuno_rulebook_golden_cases" ("rulebook_version_id", "external_case_key")`,
    );

    // ---------------------------------------------------------------
    // Governance: validation runs, review items, audit log
    // ---------------------------------------------------------------
    await queryRunner.query(`
    ...
    `);
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "idx_zuno_validation_version" ON "zuno_rulebook_validation_runs" ("rulebook_version_id", "created_at")`,
    );
    ...
    `);
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "idx_zuno_review_rule" ON "zuno_rulebook_review_items" ("rule_id")`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "idx_zuno_review_version" ON "zuno_rulebook_review_items" ("rulebook_version_id", "review_status")`,
    );
    ...
    `);
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "idx_zuno_rulebook_audit_version" ON "zuno_rulebook_audit_log" ("rulebook_version_id", "created_at")`,
    );
  }

  /**
   * Rollback. Drops the knowledge layer in reverse dependency order.
   *
   * Destroys uploaded rulebook content, so this is a development and staging
   * facility. Rolling back a *rulebook version* in production is a governance
   * action (see RulebookGovernanceService.rollback), not a schema migration.
   */

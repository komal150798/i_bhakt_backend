...
export class ManifestationEnhancedService {
  ...
  constructor(
    ...
    @InjectRepository(ManifestationProgressEntry)
    private manifestationProgressEntryRepository: Repository<ManifestationProgressEntry>,
  ...
  async createManifestation(
    ...
    // Create manifestation with quick scores first (fast response)
    const manifestation = this.manifestationRepository.create({
      ...
      action_windows: null, // Will be calculated async
      progress_tracking: {
        current_progress: 0,
        journal_entries_count: 0,
        milestones: [],
      },
      tips: null, // Will be calculated async
      insights: {
        ai_narrative: '',
        astro_insights: '',
    ...
    // ASYNC PATH: Calculate kundli-based scores and enhanced data in background
  ...
  private async enforceManifestationPlanLimit(userId: number): Promise<void> {
    const entitlements = await this.entitlementsService.getUserEntitlements(userId);
    const manifestationFeature = entitlements.features.find(
      (feature) => feature.feature === 'manifestation_journal',
    );

    if (!manifestationFeature?.allowed) {
      throw new BadRequestException(
        'Manifestation feature is not available in your current plan.',
      );
    }

    const monthlyLimit =
      manifestationFeature.limit !== undefined
    ...
    if (usage >= monthlyLimit) {
      throw new BadRequestException(
        `Monthly manifestation limit reached for your plan (${monthlyLimit}/${monthlyLimit}). Please upgrade to continue.`,
      );
    }
  }

  private getMonthlyManifestationLimit(planType: PlanType): number | null {
    if (planType === PlanType.FREE) return 3;
    if (planType === PlanType.PAID) return 12;
    if (planType === PlanType.PREMIUM) return null; // Unlimited
    if (planType === PlanType.REFERRAL) return 12;
    return 3;
  }

  /**
   * Get quick scores using rule-based analysis (no LLM, no kundli)
   * This is fast and returns immediately
   */
  private async getQuickScores(title: string, description: string): Promise<{
    category: string;
    ...
    // Quick alignment score based on specificity
    ...
    // Default astro support (will be updated async with real kundli data)
  ...
  /**
   * Enhance manifestation with full AI and kundli analysis (runs async in background)
   */
  private async enhanceManifestationAsync(
  ...
  async getDashboard(userId: number): Promise<{
    ...
    const canCreateManifestation =
      manifestationFeature?.allowed !== false &&
  ...
  /**
   * Add one daily progress entry for a manifestation.
   * Business rule: single entry per manifestation per day.
   */
  async addDailyProgressEntry(
    ...
    entryDate: string,
    actionText: string,
  ): Promise<ManifestationProgressEntry> {
    await this.ensureManifestationAccess(manifestationId, userId);

    const parsedDate = new Date(entryDate);
    if (Number.isNaN(parsedDate.getTime())) {
      throw new BadRequestException('Invalid entry date format. Use YYYY-MM-DD.');
    }
    ...
    if (existingEntry) {
      throw new BadRequestException(
        'Daily progress for this date already exists. You can edit the existing entry.',
      );
    }
  ...
  async deleteDailyProgressEntry(entryId: number, userId: number): Promise<void> {
    const entry = await this.manifestationProgressEntryRepository.findOne({
      where: { id: entryId, user_id: userId, is_deleted: false },
    });
  ...
  private async ensureManifestationAccess(
    manifestationId: number,
    userId: number,
  ): Promise<void> {
    const manifestation = await this.manifestationRepository.findOne({
  ...
  /**
   * Calculate kundli-based scores using user's actual dasha periods
   * Returns astro_support_index based on current dasha alignment with manifestation category
   */
  private async calculateKundliBasedScores(
    userId: number,
    category: string | null,
  ): Promise<{
    astro_support_index: number;
    dasha_supportive: number;
    dasha_challenging: number;
    currentDasha: {
    ...
    try {
      ...
      // Try to get dasha from kundli's dasha_timeline
      ...
      if (!currentMahadasha && kundli.nakshatra) {
        ...
        const dashaData = this.calculateDashaFromBirthDateAndNakshatra(
          birthDateTime,
          kundli.nakshatra,
          currentDate,
        );
      ...
      // Calculate dasha resonance based on category alignment
      ...
      const calculateDashaResonance = (lord: string | null): { supportive: number; challenging: number } => {
        if (!lord) return { supportive: 50, challenging: 50 };
      ...
      // Weighted average: Mahadasha 50%, Antardasha 35%, Pratyantar 15%
      const dasha_supportive = Math.round(
        mahaResonance.supportive * 0.50 +
        antaraResonance.supportive * 0.35 +
        pratyantarResonance.supportive * 0.15
      );
      const dasha_challenging = Math.round(
        mahaResonance.challenging * 0.50 +
        antaraResonance.challenging * 0.35 +
        pratyantarResonance.challenging * 0.15
      );

      // Calculate astro_support_index based on dasha alignment
  ...
  /**
   * Calculate MFP Score: Manifestation Fulfillment Probability

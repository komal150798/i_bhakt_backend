...
let RazorpayAppController = class RazorpayAppController {
    ...
    async createOrder(user, dto) {
        ...
        if (this.checkout.isDevBypassMode()) {
            const offline = await this.checkout.createOfflineSuccessForPlan(user.id, dto.plan_unique_id, billing, 'dev-offline');
            const subscription = await this.subscriptions.createSubscription(user.id, offline.plan_id, new Date(), offline.local_order_id);
            return {
                success: true,
                data: {
                    mode: 'offline-dev',

...
let KundliPdfService = KundliPdfService_1 = class KundliPdfService {
    ...
    addLagnaNakshatra(doc, data) {
        if (!data.lagna && !data.nakshatra)
            return;
    ...
    addPanchang(doc, data) {
        if (!data.tithi && !data.yoga && !data.karana)
            return;
    ...
    addDetailedDashaTable(doc, detailedTimeline, currentMaha, currentAntara, currentPratyantar) {
        doc.fontSize(11).font('Helvetica-Bold').fillColor('#1a365d');
    ...
    addDashaPageBreak(doc, colWidths, headers, headerHeight) {
        doc.addPage();
        doc.y = 50;
        let tableY = 50;
        doc.fontSize(11).font('Helvetica-Bold').fillColor('#1a365d');
    ...
    renderDashaPeriodRow(doc, period, options) {
        const { state, colWidths, rowHeight, headerHeight, currentDasha } = options;
        const isCurrentPeriod = period.mahadasha === currentDasha?.maha &&
    ...
    getDashaRowBgColor(isCurrentPeriod, mahaChanged, mahadasha) {
        if (isCurrentPeriod) {
            return '#d4edda';
        }
    ...
    drawDashaTableHeader(doc, x, y, colWidths, headers) {
    ...
    addPlanetaryPositions(doc, data) {
    ...
    addFooter(doc) {
        ...
        for (let i = 0; i < pageCount; i++) {
            ...
            doc.text(`Generated by iBhakt | ${new Date().toLocaleString()} | Page ${i + 1} of ${pageCount}`, 50, 785, { align: 'center', width: 495 });
        }
    }
...
exports.KundliPdfService = KundliPdfService = KundliPdfService_1 = __decorate([
    (0, common_1.Injectable)()
], KundliPdfService);
//# sourceMappingURL=kundli-pdf.service.js.map
...
let AdminSubscriptionsController = class AdminSubscriptionsController {
    ...
    async findAll(body) {
        ...
        if (body.search) {
            queryBuilder.andWhere('(customer.email ILIKE :search OR customer.first_name ILIKE :search OR customer.last_name ILIKE :search OR plan.name ILIKE :search)', { search: `%${body.search}%` });
        }

/**
 * Verify current Dasha periods for Rahul Gudadhe
 * 
 * As of 2025-01-26, we should be in:
 * - Mahadasha: Moon (started 2021-12-14)
 * - Antardasha: Jupiter (expected)
 * - Pratyantar: Rahu (expected)
 */
...
function verifyCurrentDasha() {
  ...
  let currentAntar: { lord: string; start: number; end: number; index: number } | null = null;
  let currentPratyantar: { lord: string; start: number; end: number; index: number } | null = null;
  ...
  if (currentMaha.lord === 'Moon') {
    ...
    // Calculate Antardashas within Moon Mahadasha
    // Moon Mahadasha duration = 10 years
    // Moon's index in DASHA_SEQUENCE = 3
    const mahaLordIndex = 3; // Moon
    const mahaDuration = 10; // years
    ...
    if (currentAntar) {
      ...
      // Calculate Pratyantars within current Antardasha
